...
function Dropdown ({text, items} ) {
  ...
  const listItems = items.map((item) => <li key={item.id} className={`flex h-0 m-5 rounded-l bg-no-repeat bg-cover w-full items-center justify-center toDown relative bg-${item.img} h-0`}><Link className="p-5 text-neutral-100 bg-neutral-900 text-xl  text-center self-center items-center z-50 w-full hidden a" to={"/itemList/" + item.categoria}>{item.categoria}</Link></li>);
  return (
    <>
        <button onClick={clickear} className="text-neutral-100 bg-neutral-700 hover:bg-yellow-500 hover:text-yellow-100 focus:outline-none  rounded-lg text-sm p-2  relative" type="button"> {text || "a"}  </button>
        <div className='z-10 bg-neutral-100 rounded divide-y divide-gray-100 shadow dropDown absolute w-full h-0 p-5 left-0 top-32 transition ease-in duration-200'>
